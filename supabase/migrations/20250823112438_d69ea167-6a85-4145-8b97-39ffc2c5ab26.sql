-- Fix infinite recursion in RLS policies by properly handling existing policies
...
-- Drop and recreate the problematic policies that cause recursion
DROP POLICY IF EXISTS "Admins can manage profiles in their company" ON public.profiles;
DROP POLICY IF EXISTS "Users can view profiles in their company" ON public.profiles;

-- Create new admin policies using security definer functions (no recursion)
...
-- Update OTP expiry to be more secure (30 minutes)
