...
BEGIN
  ...
  -- Create processing log entry (only if table exists)
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN
    log_id := NULL; -- Table doesn't exist yet, skip logging
  END;
...
DROP TRIGGER IF EXISTS trg_grn_inventory_processing_enhanced ON public.grn_header;
...
BEGIN
  -- Find the GRN ID for RIGEGRN09022025006
