-- Enhanced GRN inventory processing function with detailed logging (fixed variable names)
...
BEGIN
  ...
  -- Create processing log entry (if table exists)
  BEGIN
  ...
  BEGIN
    ...
    -- Update processing log with success (if logging enabled)
    IF log_id IS NOT NULL THEN
      UPDATE public.grn_processing_log 
  ...
  EXCEPTION WHEN OTHERS THEN
    error_msg := SQLERRM;
    -- Update processing log with error (if logging enabled)
    IF log_id IS NOT NULL THEN
      UPDATE public.grn_processing_log 
      SET processing_status = 'failed',
