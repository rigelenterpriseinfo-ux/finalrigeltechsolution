...
ADD COLUMN warehouse_id UUID REFERENCES public.warehouse_bins(warehouse_id),
...
DECLARE
    ...
    invoice_id UUID;
BEGIN
    -- Get the first available warehouse and bin
    SELECT w.id, wb.id INTO default_warehouse_id, default_bin_id
    FROM public.warehouse_bins wb
    JOIN public.warehouses w ON wb.warehouse_id = w.id
    WHERE w.company_id = (
    ...
    IF default_warehouse_id IS NOT NULL AND invoice_id IS NOT NULL THEN
